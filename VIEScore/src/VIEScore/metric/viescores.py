...
class VIEScore:
    def __init__(self, backbone="gpt4o", task="t2i", key_path=None) -> None:
        ...
        if self.backbone_name == "gpt4o":
        ...
        elif self.backbone_name == "gpt4omini":
            from ..mllm_tools import GPT4omini
            self.model = GPT4omini(key_path)
        elif self.backbone_name == "gpt4v":
            from mllm_tools.openai import GPT4v
            self.model = GPT4v(key_path)
        elif self.backbone_name == "gpt4o-mini":
            from mllm_tools.openai import GPT4oMini
            self.model = GPT4oMini(key_path)
            ...
            self.model = MiniCPMV()
        elif self.backbone_name == "qwen":
            from ..mllm_tools.qwen_eval import QwenVL
            self.model = QwenVL()
        else:
    ...
    def evaluate(self, image_prompts, text_prompt, extract_overall_score_only=False, extract_all_score=True, echo_output=False):
        ...
        if self.backbone_name in ['gpt4o', 'gpt4v', 'gpt4omini']:
        ...
        fine_grained_scores['edit_success'] = results_dict['SC']['score'][0]
        fine_grained_scores['overedit'] = results_dict['SC']['score'][1]
        PQ_score = min(results_dict['PQ']['score'])
        fine_grained_scores['looks_natural'] = results_dict['PQ']['score'][0]
        fine_grained_scores['no_artifact'] = results_dict['PQ']['score'][1]
        ...
        if extract_overall_score_only:
            return O_score
        return [SC_score, PQ_score, O_score], fine_grained_scores, results_dict
