...
def eval_automatic(OUR_MODEL_ARGS):
    ...
    source_dict = {"All_Sizes": [source for source_list in source_dict.values() for source in source_list]}
    ...
    for img_size_index, img_size in tqdm(enumerate(images_dict), desc="Processing batches with different image sizes"):
        ...
        for batch_idx, i in tqdm(enumerate(range(0, len(input_images), BATCH_SIZE)), desc="Processing batches"):
            ...
            with torch.inference_mode():
                ...
                for img, instr, source in zip(batch_input_images, batch_edit_instructions, batch_sources):
            ...
            columns = ["overview", "original_image", "edit_instruction", "reasoning", "edited_image", "grounding_image", "source"]
