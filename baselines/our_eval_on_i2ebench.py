...
from benchmarks import init_i2ebench
...
def eval_automatic(OUR_MODEL_ARGS):
    ...
    #########################################################
    # Group by task and sample from each task
    # task_groups = {}
    # for sample in data:
    #     task = sample["task"]
    ...
    # num_tasks = len(tasks)
    # samples_per_task = max(1, 500 // num_tasks)
    ...
    #     num_to_sample = min(samples_per_task, len(task_data))
    #     # Use random seed for reproducibility
    #     np.random.seed(42)
    #     sampled_indices = np.random.choice(len(task_data), num_to_sample, replace=False)
    #     for idx in sampled_indices:
    #         sampled_data.append(task_data[idx])
    
    # # If we haven't reached the desired sample size, add more samples
    # if len(sampled_data) < 500:
    #     remaining = 500 - len(sampled_data)
    #     # Flatten all available samples
    #     all_samples = [s for samples in task_groups.values() for s in samples if s not in sampled_data]
    #     if all_samples:
    #         np.random.seed(42)
    #         additional_samples = np.random.choice(all_samples, min(remaining, len(all_samples)), replace=False)
    ...
    task_type_dict = {"All_Sizes": [task_type for task_type_list in task_type_dict.values() for task_type in task_type_list]}
    ...
    for img_size_index, img_size in tqdm(enumerate(images_dict), desc="Processing batches with different image sizes"):
        ...
        for batch_idx, i in tqdm(enumerate(range(0, len(input_images), BATCH_SIZE)), desc="Processing batches"):
            ...
            with torch.inference_mode():
                ...
                for img, instr, task_type in zip(batch_input_images, batch_edit_instructions, batch_task_type):
            ...
            for batch_sample_idx in tqdm(range(len(batch_input_images)), desc="Processing batches for wandb"):
                ...
                wandb_report_row = {
                    ...
                    "task_type": batch_task_type[batch_sample_idx],
                ...
                wandb_batch_table_rows.append(
                    [
                        ...
                        batch_task_type[batch_sample_idx],
            ...
            columns = ["overview", "original_image", "edit_instruction", "reasoning", "edited_image", "grounding_image", "task_type"]
