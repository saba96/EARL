...
from benchmarks import init_magicbrush
...
def eval_automatic(baseline_name):
    ...
    BENCHMARK = "MagicBrush"
    EVAL_TABLE = "evaluation_table"

    data = init_magicbrush()
    # data = data.shuffle(seed=42).select(range(520))
    ...
    for sample in data:
        ...
        if img_size not in list(image_dict.keys()):
            ...
            gt_edited_image_dict[img_size] = [output_image.convert("RGB")]
        else:
            image_dict[img_size].append(image.convert("RGB"))
            instruction_dict[img_size].append(instruction)
            gt_edited_image_dict[img_size].append(output_image.convert("RGB"))
    ...
    if baseline in ["InstructPix2Pix_XL", "MagicBrush_XL", "Aurora_XL", "InstructPix2Pix", "MagicBrush", "Aurora"]:
        ...
        gt_edited_images_dict = {"All_Sizes": [output_image for output_image_list in gt_edited_image_dict.values() for output_image in output_image_list]}
    else:
        images_dict = image_dict
        instructions_dict = instruction_dict
        gt_edited_images_dict = gt_edited_image_dict
    ...
    for img_size_index, img_size in tqdm(enumerate(images_dict), desc="Processing batches with different image sizes"):
        ...
        for batch_idx, i in tqdm(enumerate(range(0, len(input_images), BATCH_SIZE)), desc="Processing batches"):
            ...
            columns = ["overview", "original_image", "edit_instruction", "edited_image", "gt_edited_image"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--baseline", nargs="+", help="Models to use for evaluation")
