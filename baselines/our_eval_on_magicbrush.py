...
def eval_automatic(OUR_MODEL_ARGS):
    ...
    gt_edited_image_dict = {"All_Sizes": [output_image for output_image_list in gt_edited_image_dict.values() for output_image in output_image_list]}
    ...
    for img_size_index, img_size in tqdm(enumerate(images_dict), desc="Processing batches with different image sizes"):
        ...
        gt_edited_images = gt_edited_image_dict[img_size]
        for batch_idx, i in tqdm(enumerate(range(0, len(input_images), BATCH_SIZE)), desc="Processing batches"):
            ...
            with torch.inference_mode():
                ...
                for img, instr, gt_edited_image in zip(batch_input_images, batch_edit_instructions, batch_gt_edited_images):
